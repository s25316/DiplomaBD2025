...
{
    public class GetPersonRecruitmentsHandler : IRequestHandler<GetPersonRecruitmentsRequest, ItemsResponse<RecruitmentDataDto>>
    {
        // Properties 
        ...
        // Methods
        public async Task<ItemsResponse<RecruitmentDataDto>> Handle(GetPersonRecruitmentsRequest request, CancellationToken cancellationToken)
        {
            ...
            var dbPerson = selectResult.First().Person.Person;
            dbPerson.Urls = selectResult.First().Person.Urls;
            dbPerson.PersonSkills = selectResult.First().Person.Sklills;

            var totalCount = selectResult.FirstOrDefault()?.TotalCount ?? 0;
            var items = new List<RecruitmentDataDto>();
            foreach (var item in selectResult)
            {
                ...
                items.Add(new RecruitmentDataDto
                {
        ...
        // Static Methods
        private static ItemsResponse<RecruitmentDataDto> PrepareResponse(
            HttpCode code,
            IEnumerable<RecruitmentDataDto> items,
            int totalCount)
        {
            return ItemsResponse<RecruitmentDataDto>.PrepareResponse(code, items, totalCount);
        }
